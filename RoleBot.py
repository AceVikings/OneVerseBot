...
def getTokensByDiscordId(discordId="0"):
    url = "https://oneverse-discord-bot.herokuapp.com/admin/" + str(
        discordId);
...
def getUsers(page=1):
    url = "https://oneverse-discord-bot.herokuapp.com/admin/get?page=" + str(page)
    resp = requests.get(url)
    return resp.json()
...
def createUser(token="", discordId=""):
    ...
    resp = requests.post(
        "https://oneverse-discord-bot.herokuapp.com/admin/addUser",
        json=data);
